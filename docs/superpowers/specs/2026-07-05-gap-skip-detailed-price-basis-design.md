...
```
区间内买档(价降序):0.2800×30→系数0.75[高位] · 0.2700×20→系数0.5[高位] · 0.1500×400→系数20;最大断层 12¢→规则1(宽断层);高位系数和 0.75+0.5=1.25 < 门槛20 → 整市场不挂;系数=挂量÷(最低份数×金额数值);奖励区间[0.1200,0.3000];来源:CLOB get_orderbook
```
