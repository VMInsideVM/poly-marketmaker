# probe_pnl.py — 每日盈亏台账 Phase 0 实盘探针（非 pytest，只读，不下单）。
#
# 用法：
#   1) 公开部分（无需密码，! 也能跑）：python probe_pnl.py
#        —— 只用 funder 地址查 Data API /activity（免鉴权），
#           打印 REWARD/REDEEM/DEPOSIT/WITHDRAWAL/TRADE 等各类记录的真实字段形态。
#   2) 鉴权部分（需私钥，请在你自己的终端跑，交互输密码、不进对话记录）：
#        python probe_pnl.py            （交互提示输入密码）
#        或  set PROBE_PW=你的密码 && python probe_pnl.py   （Windows，一次性）
#        —— 额外打印 /rewards/user/total（权威奖励）+ /rewards/user（earnings/asset_rate）
#           + get_trades 逐笔 fee_rate_bps（算 taker 手续费用）。
#
# 完整结果写盘 probe_pnl_dump.json。把终端输出（或该文件）贴回来即可锁定字段形态。
import os
import json
import hashlib
from collections import defaultdict
from datetime import datetime, timezone, timedelta

from models.database import Database
from utils.crypto import decrypt, derive_key
from config import DB_PATH
from py_clob_client_v2.clob_types import TradeParams
from api.polymarket_api import PolymarketAPI, DATA_API_HOST
from api.proxy import use_proxy, http_get, parse_proxy


def _utc_date(days_ago: int) -> str:
    return (datetime.now(timezone.utc) - timedelta(days=days_ago)).strftime("%Y-%m-%d")


def _dump(obj, n=2):
    """紧凑打印前 n 条样本。"""
    return json.dumps(
        obj[:n] if isinstance(obj, list) else obj, ensure_ascii=False, indent=2
    )


def _get_password(db):
    """拿密码：优先环境变量 PROBE_PW，否则交互 input；! 非交互(EOF)时返回 None。"""
    pw_hash, salt = db.get_password()
    if pw_hash is None:
        print("  (未设置密码，跳过鉴权部分)")
        return None
    pw = os.environ.get("PROBE_PW")
    if not pw:
        try:
            pw = input("请输入访问密码（直接回车/非交互则跳过鉴权部分）: ").strip()
        except EOFError:
            pw = ""
    if not pw:
        return None
    key = derive_key(pw, salt)
    if hashlib.sha256(key).hexdigest() != pw_hash:
        print("  ⚠️ 密码错误，跳过鉴权部分")
        return None
    return key


def main():
    db = Database(DB_PATH)
    db.init()
    wallets = db.list_wallets()
    if not wallets:
        print("无钱包")
        return
    w = wallets[0]  # 用第一个钱包探针（够看字段形态；多钱包口径一致）
    funder = w.get("funder") or ""
    proxy_url = parse_proxy(w.get("proxy") or "")
    print("=" * 78)
    print(
        f"钱包 funder = {funder} | signature_type = {w.get('signature_type')}"
        f" | 代理 = {'有' if proxy_url else '直连'}"
    )
    print("=" * 78)
    out = {"funder": funder}

    # ================= 公开部分：/activity（无需密码/鉴权） =================
    print("\n【2) 活动流 /activity（免鉴权，按 type 分组取样）】")
    activity = []
    try:
        with use_proxy(proxy_url):
            activity = http_get(
                f"{DATA_API_HOST}/activity",
                params={"user": funder, "limit": 500},
                timeout=20,
            ).json()
    except Exception as e:
        print(f"  /activity 报错: {e}")
    by_type = defaultdict(list)
    for a in activity if isinstance(activity, list) else []:
        by_type[a.get("type", "?")].append(a)
    print(
        f"  共 {len(activity)} 条；type 分布: {({t: len(v) for t, v in by_type.items()})}"
    )
    for t in (
        "REWARD",
        "REDEEM",
        "DEPOSIT",
        "WITHDRAWAL",
        "TRADE",
        "MERGE",
        "SPLIT",
        "CONVERSION",
    ):
        if by_type.get(t):
            print(
                f"\n  -- type={t}（{len(by_type[t])} 条，样本）--\n{_dump(by_type[t], 2)}"
            )
    out["activity_by_type"] = {t: v for t, v in by_type.items()}

    # ================= 鉴权部分：奖励 + 逐笔手续费（需私钥） =================
    key = _get_password(db)
    if key is None:
        print(
            "\n【1/3) 鉴权部分已跳过】权威奖励与逐笔手续费需私钥；请在你自己的终端跑："
            "\n    python probe_pnl.py   （交互输密码，不进对话记录）"
        )
    else:
        private_key = decrypt(w["encrypted_key"], key)
        api = PolymarketAPI(
            private_key,
            signature_type=w.get("signature_type", 2),
            funder=w.get("funder") or None,
            proxy=w.get("proxy") or None,
        )
        with use_proxy(api.proxy_url):
            print("\n【1) 做市奖励 /rewards/user/total（按天，UTC 日期）】")
            rewards_total = {}
            for d_ago in (1, 2, 3, 4):
                date = _utc_date(d_ago)
                try:
                    r = api.client.get_total_earnings_for_user_for_day(date)
                    rewards_total[date] = r
                    print(f"  {date}: {json.dumps(r, ensure_ascii=False)}")
                except Exception as e:
                    print(f"  {date}: 报错 {e}")
            out["rewards_total"] = rewards_total

            print("\n【1b) 做市奖励 /rewards/user（按市场，看 earnings/asset_rate）】")
            rewards_by_market = {}
            for d_ago in (1, 2, 3):
                date = _utc_date(d_ago)
                try:
                    r = api.client.get_earnings_for_user_for_day(date)
                    rewards_by_market[date] = r
                    print(f"  {date}: {len(r)} 条；样本:\n{_dump(r, 2)}")
                    if r:
                        break
                except Exception as e:
                    print(f"  {date}: 报错 {e}")
            out["rewards_by_market"] = rewards_by_market

            print(
                "\n【3) get_trades 逐笔成交（fee_rate_bps / maker_orders 费率字段）】"
            )
            try:
                trades = api.get_trades(TradeParams(maker_address=funder))
                print(f"  共 {len(trades)} 笔；前 3 笔:\n{_dump(trades, 3)}")
                out["trades_sample"] = trades[:5]
            except Exception as e:
                print(f"  get_trades 报错: {e}")

    with open("probe_pnl_dump.json", "w", encoding="utf-8") as fh:
        json.dump(out, fh, ensure_ascii=False, indent=2, default=str)
    print("\n完整结果已写入 probe_pnl_dump.json（把终端输出贴回来即可）")


if __name__ == "__main__":
    main()
