...
- `engine/monitor.py` detects full/partial fills (tracks `size_matched` deltas), places matching sell orders, records positions/trades, and triggers market-sell stop-loss.
...
- **Startup recovery** (`startup_recovery`) cancels stale buy orders and detects fills that happened while offline, placing catch-up sell orders. Don't bypass it.
